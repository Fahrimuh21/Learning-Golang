...
//tentang penggunaan atomic pada goroutine
//dokumentasi sync/atomic: https://pkg.go.dev/sync/atomic
	...
	for i := 1; i <= 1000; i++ {
		go func() {
			...
			*/
			group.Add(1)
			for j := 1; j <= 100; j++ {
				atomic.AddInt64(&x, 1) //kalo struct mending mutex
				//kalo primitif mending atomic
			}
			group.Done()
		}()
	}
